 ...
 * PIPELINE's declared steps, and driven by the SERVER's scheduler — which
 * starts each ready step as an ordinary run of the registered `deliver` graph,
 ...
 * writes: steps are data, `readiness` is the RED precondition, `record` appends
 ...
 * the
 * assertions: the step id is in every leaf's input, so two steps cannot collide
 * on a key and one step's decision cannot replay as another's.
 *
 ...
 * scripted binding, a person approving, and `persist` writing the steps.
 */
 ...
 * measured RED. Without one the step is not ready and nothing delivers it,
...
/**
 * What each leaf a happy run reaches answers, per step.
 *
 * Scripted at the BINDING rather than seeded into the journal: the journal is
 * production replay and nothing here writes to it. Which step a call is about
...
const payloadFor = (leaf: LeafId, stepId: string, symbolId: string, body: string) => ({
  anchor: "",
  rationale: "stubbed",
  symbolId,
  body,
...
/** What each step writes: the symbol it rewrites, and the body it rewrites it to. */
type StepScript = { symbolId: string; body: string };

/**
 * Every DELIVER leaf a happy run reaches, for however many steps the caller
 * scripted. One binding serves both slots: it dispatches on the call's role,
 * so the worker answers and the validator passes.
 */
const scriptedFor = (steps: Record<string, StepScript>): ModelBinding =>
  scriptedBinding(({ step, prompt }): ScriptedAnswer[] | undefined => {
    ...
    const stepId = Object.keys(steps).find((id) => prompt.includes(id)) ?? "";
    const script = steps[stepId];
    if (script === undefined) return undefined;
    return [{ decision, payload: payloadFor(leaf, stepId, script.symbolId, script.body) }];
  });
 ...
 *
 * The graph is `deliver`, registered once; the pipeline is steps pointed at it
 * plus the two hooks. Every step's run goes through the SAME registration a
 * person starting one step by hand would reach, which is what makes "no door
...
const openServerRegistry = (spec: {
  ...
  const stepsOf = () => readRoadmap(spec.artifacts, REQUEST);
  const stepFor = (stepId: string) => {
    const step = stepsOf().find((r) => r.id === stepId);
    if (step === undefined) throw new Error(`no step ${stepId} in the roadmap`);
    return step;
  };

  const deliver = registration<State, { stepId: string }>({
    id: "deliver",
    title: "DELIVER — the step cycle over one step",
    input: z.object({ stepId: z.string().min(1) }),
    journal: spec.journal,
    graph: (ctx) => deliverGraph(ctx.journal, spec.defs, commands, ctx.observe),
    seed: (input) => {
      const step = stepFor(input.stepId);
      if (!oracleIsRed(spec.artifacts, step.id)) {
        throw new Error(`${step.id} has no oracle measured red, so nothing may deliver it.`);
      }
      return deliverSeed({ vcs: spec.vcs, step, design: DESIGN, evidence: "" });
    },
    executor: ({ runId, input }) => {
      ...
      return vcsExecutor({
        vcs: spec.vcs,
        session: runId,
        intent: { taskId: step.id, parentTaskId: REQUEST, description: step.observation },
        artifacts: spec.artifacts,
        knownRed: knownRedTests(spec.artifacts, spec.vcs, stepsOf(), step.id),
  ...
  const delivery: PipelineRegistration = {
    ...
    readiness: (stepId) => oracleIsRed(spec.artifacts, stepId),
    record: (stepId, outcome) => {
      recordStepRun(spec.artifacts, stepId, outcome);
    },
...
describe("the pipeline: roadmap steps in, two DELIVER runs out", () => {
  test("A runs first, B after A is accepted, and both acceptance tests pass at the end", async () => {
    ...
    // 2. Every leaf of every step, scripted at the binding.
    const bodies: Record<string, string> = { "01-01": IMPLEMENTED.alpha, "01-02": IMPLEMENTED.bravo };
    const scripts: Record<string, StepScript> = Object.fromEntries(
      readRoadmap(artifacts, REQUEST).map((step) => [
        step.id,
        { symbolId: step.predictedTouches[0] as string, body: bodies[step.id] as string },
      ]),
    );
    const defs = deliverDefs({ worker: scriptedFor(scripts), validator: scriptedFor(scripts) });

    // 3. DISTILL measured both oracles red, which is what makes the steps
    //    deliverable at all.
    for (const step of readRoadmap(artifacts, REQUEST)) recordRedOracle(artifacts, step.id);

    // 4. Two steps, one server, one VCS, one journal.
    ...
    // Both steps were accepted, and the dependency was honoured: B only became
    // ready because A read `accepted`.
    expect(tree.steps.map((r) => `${r.id}:${r.status}`)).toEqual(["01-01:accepted", "01-02:accepted"]);
    expect(statusOf(artifacts, "01-01")).toBe("accepted");
    expect(statusOf(artifacts, "01-02")).toBe("accepted");

    // Each step's run is an ORDINARY run: it has a server run id, a trace
    // through the authored nodes, and the step links to it.
    for (const step of tree.steps) {
      expect(step.runId).toBeDefined();
      const record = registry.runs.get(step.runId as string);
    ...
    // The durable step carries the ENGINE's run id — the one the snapshot is
    ...
    expect(runs.map((r) => r.runId).sort()).toEqual(
      tree.steps.map((r) => registry.runs.get(r.runId as string)?.engineRunId as string).sort(),
    );
    ...
    // 6. The event log shows both, under each step's own task id — and under
    //    the RUN's own session, so two runs hold two leases rather than
    //    colliding on the one a session may hold.
    for (const step of tree.steps) {
      const events = project.vcs.log.byTask(step.id);
      ...
      expect(JSON.parse(String(acquired?.detail)).session).toBe(step.runId);
    }
  ...
  test("an implementation that does not satisfy the activated test is refused by the real gate", async () => {
    ...
    const [step] = readRoadmap(artifacts, REQUEST);
    if (step === undefined) throw new Error("the fixture has two steps");
    // A body that compiles and does not make the acceptance test pass.
    const scripts: Record<string, StepScript> = {
      [step.id]: {
        symbolId: step.predictedTouches[0] as string,
    ...
    // `bun test` ran for real and refused the write, so the step parked rather
    ...
    expect(tree.steps.map((r) => r.status)).toEqual(["suspended", "pending"]);
    expect(project.read("src/alpha.ts")).toContain("return 1;");
    // The oracle is untouched, because the crafter never held it.
    expect(project.read("src/alpha.test.ts")).toBe(oracleFor("alpha", 42));

    // The parked step IS a parked run, with the enum a person answers from.
    const parked = registry.runs.get(tree.steps[0]?.runId as string);
    expect(parked?.status).toBe("suspended");
    expect(parked?.suspension?.resume?.options?.length).toBeGreaterThan(0);

    // The refusal is in the log as a verification failure, under the step's task.
  ...
  test("two steps never share a journal key, because the step id is in every leaf's input", () => {
    const project = openProject();
    ...
    const keyFor = (step: typeof a) =>
      journalKey(
        defs.implement,
        LeafInput.parse({ step: stepUnderDelivery(step, DESIGN), evidence: "", impacted: [] }),
      );

    // The same leaf, the same design, the same everything except the step. If
    // these collided, one step's `implement` decision would replay as the
    // other's and the second step would write the first step's body.
  ...
  test("the crafter's executor is built with the step's oracle protected", async () => {
...
/**
 * A declared `resources` is what makes two steps take turns.
 *
 * `run-command.resources` is where a consumer says "these tests need the
 * shared database". The server is where two steps that both need it are made to
 * take turns. The join is `declaredResources`, which asks the step's own
 * declared commands what they need — and this is the pair of runs that proves
 * the join is live: the SAME two independent steps, with and without the
 * declaration.
 */
describe("a declared resource serializes two steps", () => {
  /** Two steps that do not depend on each other, so nothing else orders them. */
  ...
  const deliverBoth = async (commands: typeof bunCommands) => {
    ...
    const bodies: Record<string, string> = { "01-01": IMPLEMENTED.alpha, "01-02": IMPLEMENTED.bravo };
    const scripts: Record<string, StepScript> = {};
    for (const step of readRoadmap(artifacts, REQUEST)) {
      scripts[step.id] = {
        symbolId: step.predictedTouches[0] as string,
        body: bodies[step.id] as string,
      };
      recordRedOracle(artifacts, step.id);
    }
    const defs = deliverDefs({ worker: scriptedFor(scripts), validator: scriptedFor(scripts) });
    ...
    const names = readRoadmap(artifacts, REQUEST).map((step) =>
      declaredResources(project.vcs, commands, step),
    );
    const tree = await deliverAll(registry);
    artifacts.close();
    project.vcs.close();
  ...
  test("the resource names come off the step's own declared commands", async () => {
    const shared = {
      ...bunCommands,
    ...
    expect(tree.steps.map((r) => r.status)).toEqual(["accepted", "accepted"]);
    // The name came off the declaration, not off the registration's options.
    expect(names).toEqual([["shared-db"], ["shared-db"]]);
  }, 60_000);

  test("two steps whose commands name nothing declare nothing", async () => {
    // The control. Same steps, same concurrency; the only difference is what
