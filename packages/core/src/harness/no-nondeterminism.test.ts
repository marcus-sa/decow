...
/** `<repo>/targets`, where a consumer's composition declares its commands. */
 ...
 * steps, and it is read on every write: a declaration that built a temp path
 * out of a clock, or a seed out of an RNG, would make the same write produce a
 * different command on every run, which is the property this whole scan exists
 * to hold. The glob is `targets/*` rather than `targets/todo` so a second
 * target is scanned the day it lands, and it names `.des/` because that is
 * where a composition's configuration lives — `dot: true`, because the scan
 * skips hidden directories otherwise.
 *
...
const scanned = async (): Promise<string[]> => {
  ...
  for await (const match of new Glob("*/.des/commands.ts").scan({
    cwd: TARGETS,
    absolute: true,
    dot: true,
  })) {
...
describe("no nondeterminism inside the graph", () => {
  test("the scanned set is non-empty and includes the runner and every graph", async () => {
    ...
    // The consumer composition's own declaration of how its project is built
    // and tested.
    expect(files).toContain("targets/todo/.des/commands.ts");
  ...
  test("a composition's declared commands are scanned, and a clock in one would fail", async () => {
    ...
    const declared = await Bun.file(join(TARGETS, "todo", ".des", "commands.ts")).text();
    expect(BANNED.some((b) => stripComments(declared).includes(b))).toBe(false);
    expect((await scanned()).some((f) => f.endsWith("targets/todo/.des/commands.ts"))).toBe(true);
  });
